        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg overflow-hidden mx-5">
            ...
            <form action="{{ route('feed.store') }}" method="POST" class="space-y-4 mx-8 my-6">
                ...
                <div>
                    ...
                    <select id="feed_type" name="feed_type" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200">
                        <option value="Herbivore" {{ old('feed_type') == 'Herbivore' ? 'selected' : '' }}>Herbivore</option>
                        <option value="Carnivore" {{ old('feed_type') == 'Carnivore' ? 'selected' : '' }}>Carnivore</option>
                        <option value="Omnivore" {{ old('feed_type') == 'Omnivore' ? 'selected' : '' }}>Omnivore</option>
                    </select>
                    @error('feed_type')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <!-- Submit Button -->
                <div>
                    <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition duration-200 ease-in-out">
                        Save Feed
                    </button>
                </div>
            </form>
